  ...
  return (
    <div className="container mx-auto px-4">
      <header className="bg-darkGreen text-white py-4">
        <h1 className="text-4xl font-bold text-center">Diagnoself</h1>
      </header>
      ...
      <main className="mt-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mt-6">
            <label htmlFor="formOtherSymptoms" className="block text-sm text-center font-medium text-gray-700">
              Other Symptoms
            </label>
            <textarea
          ...
          <div className="flex justify-center mt-3">
            <button
              type="submit"
              className="bg-blue-500 text-white py-2 px-4  mr-3 rounded disabled:opacity-50"
              disabled={loading}
            >
          ...
          {conditions.length > 0 ? (
            <div className="flex flex-wrap -mx-2">
              {conditions.map((condition, index) => (
                <div
                  key={index}
                  className={`w-full ${index === 0 || index === conditions.length - 1 ? 'md:w-full' : 'md:w-1/2'} px-2 mb-3`}
                >
                  <div className={`bg-white shadow-md rounded-lg p-4 ${index !== 0 && index !== conditions.length - 1 ? 'bg-gray-100' : ''}`}>
